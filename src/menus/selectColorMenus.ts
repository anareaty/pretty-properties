...
const createColorMenu = (item: MenuItem, pillVal: string, plugin: PrettyPropertiesPlugin) => {
    item.setTitle(i18n.t("SELECT_COLOR"))
        .setIcon("paintbrush")
        .setSection("pretty-properties");
    //@ts-ignore
    let sub = item.setSubmenu() as Menu;
    setColorMenuItems(sub, pillVal, "propertyPillColors", plugin)
}
    ...
    if (pillEl instanceof HTMLElement) {
        ...
        if (pillVal) {
            menuManager.addItemAfter(
                ...
                i18n.t("SELECT_COLOR"),
                (item: MenuItem) => {
                    if (pillVal) createColorMenu(item, pillVal, plugin)
                }
            );
        }
    } 
    ...
    if (tag instanceof HTMLElement) {
        ...
        if (tagText) {
            menuManager.addItemAfter(
                ...
                i18n.t("SELECT_COLOR"),
                (item: MenuItem) => {
                    if (tagText) createColorMenu(item, tagText, plugin)
                }
            );
        }
    }
